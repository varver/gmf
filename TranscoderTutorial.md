the following example show an overview of an input file with one video and two audiostreams
```
                                    ---------------   --------------   -------------   --------------
                                |---|VideoStream-1|---|VideoDecoder|---|VideoFilter|---|VideoEncoder|---|
                                |   ---------------   --------------   -------------   --------------   |
-----------   ---------------   |   ---------------   --------------   -------------   --------------   |   -------------   ------------
|InputFile|---|Demultiplexer|---|---|AudioStream-1|---|AudioDecoder|---|AudioFilter|---|AudioEncoder|---|---|Multiplexer|---|OutputFile|
-----------   ---------------   |   ---------------   --------------   -------------   --------------   |   -------------   ------------
                                |   ---------------   --------------   -------------   --------------   |
                                |---|AudioStream-2|---|AudioDecoder|---|AudioFilter|---|AudioEncoder|---|
                                    ---------------   --------------   -------------   --------------
```
...
```
  1. setup the demultiplexer
```
demultiplexer:=gmf.NewDemultiplexer(source)
```
  1. prepare the outputfile
```
...
```
  1. process all streams in from the input file (the function process\_track is defined below)
```
...
```
  1. starting the Multiplexer and Demultiplexer, the call to the Demultiplexer will block until all Packets are readed from the inputstreams
```
multiplexer.Start()
demultiplexer.Start()
```
  1. finalize the transcoding and clean up
```
multiplexer.Stop()
demultiplexer.Stop()
source.Disconnect()
